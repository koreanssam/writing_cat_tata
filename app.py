...
generate_field = st.container(border=True)

# 모델 선택
uploade_field.subheader("📝 따라 쓸 언어를 선택하세요.")
model_selection = uploade_field.selectbox("", ("랜덤 명대사 생성을 위한 언어를 선택하세요.", "영어", "한국어"), index=0, key="language_selection")

# 명대사 출력
quote = None
if model_selection == "영어":
    quote = random_quote_en()
elif model_selection == "한국어":
    quote = random_quote_ko()
else:
    quote = ""

if quote:
    uploade_field.markdown(f"<div style='background-color: #F0F8FF; padding: 10px; border-radius: 10px;'><h2 style='text-align: left; color: #000000; font-family: -apple-system, BlinkMacSystemFont, 'Apple SD Gothic Neo', 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;'>{quote}</h2></div>", unsafe_allow_html=True)
    st.session_state["selected_quote"] = quote  # 선택된 명대사를 세션 상태에 저장

# 이미지 업로드
uploade_field.subheader("🗃️ 이미지 업로드")
with uploade_field.form("my-form", clear_on_submit=True, border=False):
    uploaded_file = st.file_uploader("이미지를 업로드하세요 (png, jpg, jpeg 형식)", 
                                     type=["png", "jpg", "jpeg"], 
